   ...
   const [errors, setErrors] = useState({
    ...
    <div>
        ...
        <p className={style.errors}>{errors.email}</p>
          ...
          <input name="repeatPassword" type="password" placeholder="Repeat Password" value={userInfo.repeatPassword} onChange={handleChange}/>
          <p className={style.errors}>{errors.repeatPassword}</p>

          <button className={style.formButton} type="submit" disabled={!userInfo.email || !userInfo.password || !userInfo.repeatPassword || errors.repeatPassword.length >1|| errors.email.length >1 || errors.password.length > 1}>Create</button>
